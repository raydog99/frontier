...
let create ~base_transformer ~memory_size ~compression =
  let module QAS = (val base_transformer.qas_space) in
  
  let params = {
    width = compression.width;
    depth = compression.depth;
    head_dim = compression.width / 8;
    num_heads = 8;
  } in
  
  let hypernetwork = create_hypernetwork params compression in
  
  let initial_memory = {
    parameters = Tensor.zeros [compression.parameters];
    velocity = Tensor.zeros [compression.parameters];
    momentum = 0.9;
    time = 0;
  } in
...
let evolve_parameters state time =
  let evolution_rate = 
    exp (-. float_of_int (abs (state.time - time))) in
  
  let new_velocity =
    Tensor.add
      (Tensor.mul_scalar state.velocity state.momentum)
      (Tensor.mul_scalar state.parameters 
         ((1. -. state.momentum) *. evolution_rate)) in
  
  let new_params =
    Tensor.add state.parameters 
      (Tensor.mul_scalar new_velocity evolution_rate) in
